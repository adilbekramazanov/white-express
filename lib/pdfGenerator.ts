...
async function buildBarcode(text: string): Promise<HTMLCanvasElement | null> {
  try {
    ...
    bwipjs.toCanvas(canvas, {
      bcid: "code128",
      text,
      scale: 6,      // ~2× the previous scale of 4 → much taller bars
      height: 28,    // bar height in bwip units (was 16)
      includetext: true,
      textxalign: "center",
      textsize: 16,  // number text below bars (was 12)
      textgaps: 2,
    });
    return canvas;
  } catch {
...
async function renderLabelCanvas(data: LabelData): Promise<HTMLCanvasElement> {
  ...
  const BC_MAX_W = CW;
  const BC_MAX_H = mm(28);  // reduced from 42 to make room for the larger logo
  ...
  ctx.font = `bold ${pt(10)}px ${FONT}`;  // was pt(7.5)
  ctx.textAlign = "left";
  ...
  const LABEL_COL   = mm(30);          // was mm(24) — wider to fit longer Cyrillic labels
  const VALUE_MAX_W = CW - LABEL_COL - mm(3);
  const ROW_FONT_PX = pt(12);          // was pt(9)
  const LABEL_FONT_PX = pt(9);         // was pt(6.5)
  const LINE_H  = mm(7);              // was mm(5)
  const ROW_PAD = mm(3);              // was mm(4)
  ...
  fields.forEach(({ label, value }, idx) => {
    ...
    const rowH = Math.max(lines.length * LINE_H, mm(10));
